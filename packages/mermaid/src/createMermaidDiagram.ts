...
export async function createMermaidDiagram(
	...
	try {
		const parsedSvg = (await mermaid.render(`mermaid-${nextDiagramId++}`, text, offscreen)).svg
		...
		switch (parsedResult.diagramType) {
			case 'flowchart-v2': {
				...
				const blueprint = flowchartToBlueprint(liveSvg, vertices, edges, subGraphs)
				renderBlueprint(editor, blueprint)
				break
			}
			case 'sequence': {
				const db = diagramResult.db as SequenceDB
				const blueprint = sequenceToBlueprint(
					liveSvg,
					db.getActors(),
					db.getActorKeys(),
					db.getMessages(),
			...
			case 'stateDiagram': {
				...
				const blueprint = stateToBlueprint(liveSvg, states, relations)
				renderBlueprint(editor, blueprint)
				break
			}
			default:
				await onUnsupportedDiagram?.(parsedSvg)
